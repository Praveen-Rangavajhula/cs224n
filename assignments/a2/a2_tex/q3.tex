...
\begin{parts}
    ...
    \ifans{
        A sentence with $n$ words will be parsed in approximately $2n$ steps.
        Each word must first be pushed onto the stack, requiring
        $n$ \texttt{SHIFT} operations. Subsequently, every word must be
        removed through either a \texttt{LEFT-ARC} or \texttt{RIGHT-ARC}
        operation, which also occurs exactly $n$ times. Therefore, the
        total number of transitions required for parsing is $2n$.
    }
   ...
   In \texttt{parser\_model.py} you will find skeleton code to implement this simple neural network using PyTorch. Complete the \texttt{\_\_init\_\_}, \texttt{embedding\_lookup} and \texttt{forward} functions to implement the model. Then complete the \texttt{train\_for\_epoch} and \texttt{train} functions within the \texttt{run.py} file.
    ...
    \begin{enumerate}[label=\roman*.]
        \item {
            ...
            First, since ReLU is applied element-wise:
            $$
            h_i = \text{ReLU}(z_i) =
            \begin{cases}
            z_i & \text{if } z_i > 0 \\
        ...
        \item {
            \textbf{Final UAS Scores:}

            After training and evaluating the neural dependency parser, the best Unlabeled Attachment Scores (UAS) achieved are as follows:

            \begin{itemize}
                \item \textbf{Best UAS on Dev Set:} 87.96
                \item \textbf{Final UAS on Test Set:} 88.12
            \end{itemize}

            \textbf{Training Details:}
            \begin{itemize}
                \item Loss on train set: 0.0673
                \item Model successfully passed all sanity checks (\texttt{-e} and \texttt{-f} flags).
                \item Training was performed using the provided hyperparameters.
            \end{itemize}

            The final UAS scores meet the expected performance criteria outlined in the assignment.
        }
...
\begin{enumerate}[label=\roman*.]
    ...
    \item {
        ...
        \item \textbf{Correct dependency}: crucial $\rightarrow$ success
        \end{itemize}
    }
    \item {
        \begin{itemize}
...
\ifans{
    The benefit of using part-of-speech (POS) tagging in a parser is that it helps resolve
    ambiguity in the role of a word within a sentence. For example, a word like "run"
    can function as either a noun or a verb, and relying only on the word itself may
    lead to incorrect parsing. By including POS tags, the model can learn not just from
    the word but also from the sentence structure, making parsing more efficient.

    Additionally, POS tagging is especially useful when encountering rare or unseen words.
    Even if the parser does not recognize a specific word, knowing its POS tag allows
    it to generalize based on similar words it has seen before. This improves the model's
    ability to correctly analyze sentence structure and assign dependencies more accurately.
}
